...
export default function HomePage() {
  ...
  // Calcoli veloci
  const owedToMe = ious.filter(i => !i.paid && i.amount > 0).reduce((s, i) => s + Number(i.amount), 0);
  const iOwe = ious.filter(i => !i.paid && i.amount < 0).reduce((s, i) => s + Number(i.amount), 0);
    ...
    <div className="min-h-screen bg-[#0f172a] text-white p-6">
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="p-4 rounded-xl bg-green-900/40 border border-green-700">
          ...
          <p className="text-2xl font-bold text-green-400">{owedToMe.toFixed(2)}</p>
        </div>

        <div className="p-4 rounded-xl bg-yellow-900/40 border border-yellow-700">
          <p className="text-sm text-yellow-300">I Owe</p>
          <p className="text-2xl font-bold text-yellow-400">{Math.abs(iOwe).toFixed(2)}</p>
        </div>
      </div>
      ...
      {!loading && !hasIOUs && (
        <p className="text-gray-500 text-center mt-10">
          No active IOUs<br />Try adding a new one.
        </p>
      )}
