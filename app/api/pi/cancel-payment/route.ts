...
async function cancelOnPi(paymentId: string) {
  if (!PI_API_KEY) {
    throw new Error("PI_API_KEY is not configured in environment variables.");
  }

  const res = await fetch(`${PI_API_BASE_URL}/v2/payments/${paymentId}/cancel`, {
    method: "POST",
    headers: {
  ...
  if (!res.ok) {
    throw new Error(`Pi cancel failed with status ${res.status}`);
  }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const piPayment = await cancelOnPi(paymentId);
